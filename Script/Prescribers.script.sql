...
-- SELECT CONCAT(nppes_provider_last_org_name,',',' ',nppes_provider_first_name) AS nnpes_provider_name, npi, SUM(total_claim_count) AS total_claims
-- FROM prescriber
-- INNER JOIN prescription 
-- USING (NPI)
-- WHERE total_claim_count IS NOT NULL
-- 	GROUP BY nnpes_provider_name, npi
-- ORDER BY SUM(total_claim_count) DESC;

--ANSWER: Bruce Pendley NPI - 1881634483, Total claims - 99,707
...
-- USING (NPI)
-- WHERE total_claim_count IS NOT NULL
-- 	GROUP BY nnpes_provider_name, npi, specialty_description
-- ORDER BY SUM(total_claim_count) DESC;
...
-- SELECT generic_name, ROUND(SUM(total_drug_cost) / 365, 2)
-- FROM prescription
-- INNER JOIN drug
-- USING (drug_name)
-- GROUP BY generic_name
...
--ANSWER: "INSULIN GLARGINE,HUM.REC.ANLOG" - $285654.98
...
-- SELECT CONCAT(nppes_provider_last_org_name,',',' ',nppes_provider_first_name) AS nnpes_provider_name,
...
-- SELECT CONCAT(nppes_provider_last_org_name,',',' ',nppes_provider_first_name) AS nnpes_provider_name,
-- 		npi, drug_name, COALESCE(total_claim_count,0) AS total_claim_coalesce
-- FROM prescriber
-- CROSS JOIN drug
-- LEFT JOIN prescription
-- USING (npi, drug_name)
